{-# LANGUAGE CPP, DataKinds, RecordWildCards, TemplateHaskell, TypeOperators #-}
...
data DhSignRequest  = DhSignRequest      { dhSgnUser :: T.Text, dhClSig :: String }
...
runDHServer' = do
  ...
  io <- newIORef $ DhData (LRU.newLRU $ Just 10000) (cprgCreate ep)
  ...
  run 8001 $ serve dhApi $ dh :<|> sign
    ...
    where
      ...
      sign DhSignRequest {..} = liftIO $ do
        ...
        let (lru', shared') = LRU.delete dhSgnUser $ dhLRU st'
